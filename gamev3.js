...
function calcdistance(endindex,curplayer,tryingshortcut)
{
    let startindex=0;
    ...
    if(start.shortcut==false&&tryingshortcut)
    {
    ...
    while(nodes.length>0)
    {
        ...
        for(let i=0;i<currentnode.connections.length;i++)
        {
            ...
            if(!visited[nodeindex])
            {
                ...
                {
                    console.log(`Went from ${startindex} ${board[startindex].x},${board[startindex].y} to ${endindex} ${board[endindex].x},${board[endindex].y} in ${curdis} moves!`);
                    return curdis;
                }
            }
        }
...
function move(spaceindex,curtd)
{
    ...
    for(let i=0;i<players[turn].length;i++)
    {
        ...
        let dis=calcdistance(spaceindex,players[turn][i],true);
        console.log(`P${turn+1}${i+1} ${dis}`);
        
       //Shortcut Move 
        if(board[spaceindex].shortcut&&firstmove&&dis<=totalmoves)
        {
            console.log(dis);
        ...
        dis=calcdistance(spaceindex,players[turn][i],false);
        //Regular Move
        if(dis<=totalmoves)
        {
            console.log("move");
